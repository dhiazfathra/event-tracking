...
- **Limits are enforced before compilation, not only at execution.** The ClickHouse profile bounds what the *database* will do; it does nothing about a request that exhausts the API or the compiler first. A public endpoint accepting a 50 MB body with 10,000 filters is a denial-of-service vector that never reaches ClickHouse. The gateway and `pkg/querydsl` therefore reject, before emitting any SQL: request bodies over 256 KB, more than 32 filter predicates, more than 4 breakdown dimensions, filter values longer than 1 KB, date ranges wider than the tenant's retention, and any query whose generated SQL exceeds 64 KB. These are the authoritative limits; the ClickHouse settings profile remains as defense in depth for whatever gets through.
